#!/bin/bash
# Скрипт для запуска dashboard с ngrok для внешнего доступа

echo "🔬 Запуск Dashboard для анализа патологий WSI"
echo "=============================================="
echo ""

# Проверка, что мы в правильной директории
if [ ! -f "scale/dashboard.py" ]; then
    echo "❌ Ошибка: файл scale/dashboard.py не найден"
    echo "   Запустите скрипт из корневой директории проекта"
    exit 1
fi

# Проверка ngrok
if ! command -v ngrok &> /dev/null; then
    echo "❌ ngrok не установлен"
    echo "   Установите: sudo snap install ngrok"
    echo "   Или: wget https://bin.equinox.io/c/bNyj1mQVY4c/ngrok-v3-stable-linux-amd64.tgz"
    exit 1
fi

# Проверка, что порт 8501 свободен
if lsof -Pi :8501 -sTCP:LISTEN -t >/dev/null ; then
    echo "⚠️  Порт 8501 уже занят"
    echo "   Остановите существующий процесс или измените порт"
    read -p "   Убить процесс на порту 8501? (y/n) " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        kill -9 $(lsof -t -i:8501)
        sleep 2
    else
        exit 1
    fi
fi

echo "📦 Запуск Streamlit dashboard..."
streamlit run scale/dashboard.py --server.port 8501 --server.address 0.0.0.0 &
STREAMLIT_PID=$!

# Ждем, пока Streamlit запустится
sleep 3

# Проверка, что Streamlit запустился
if ! ps -p $STREAMLIT_PID > /dev/null; then
    echo "❌ Ошибка: Streamlit не запустился"
    exit 1
fi

echo "✅ Streamlit запущен (PID: $STREAMLIT_PID)"
echo ""
echo "🌐 Запуск ngrok туннеля..."
echo ""

# Запуск ngrok в фоне
ngrok http 8501 > /tmp/ngrok.log 2>&1 &
NGROK_PID=$!

sleep 3

# Получение URL из ngrok
NGROK_URL=$(curl -s http://localhost:4040/api/tunnels | grep -o 'https://[^"]*\.ngrok[^"]*' | head -1)

if [ -z "$NGROK_URL" ]; then
    echo "⚠️  Не удалось получить URL от ngrok"
    echo "   Проверьте логи: tail -f /tmp/ngrok.log"
    echo ""
    echo "   Вы можете открыть ngrok веб-интерфейс: http://localhost:4040"
else
    echo "✅ ngrok запущен (PID: $NGROK_PID)"
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo "🎉 Dashboard доступен по адресу:"
    echo ""
    echo "   $NGROK_URL"
    echo ""
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo ""
    echo "📊 Ngrok веб-интерфейс: http://localhost:4040"
    echo ""
fi

echo "💡 Для остановки нажмите Ctrl+C"
echo ""

# Функция для очистки при выходе
cleanup() {
    echo ""
    echo "🛑 Остановка процессов..."
    kill $STREAMLIT_PID 2>/dev/null
    kill $NGROK_PID 2>/dev/null
    echo "✅ Процессы остановлены"
    exit 0
}

trap cleanup SIGINT SIGTERM

# Ожидание
wait


...
1. **Модуль `dashboard.py`** ✅ (веб-интерфейс на Streamlit):
   ...
   - **Инференс для новых WSI**:
     - Автоматическая загрузка модели из эксперимента
     - Автоматическое определение типа признаков (относительные/абсолютные)
     - Использование тех же признаков, что при обучении
     - Автоматическое добавление недостающих признаков с нулевыми значениями
     - Визуализация результатов инференса на спектральной шкале
   ...
   - Выбор директории для загрузки файлов (предустановленные или своя)

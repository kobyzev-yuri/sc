"""
Скрипт для сравнения производительности обычного метода и метода через подсекции.
Обрабатывает все WSI файлы обоими методами и показывает ускорение.
"""
...
for wsi_name in os.listdir(WSI_IMGS_DIR):
    ...
    try:
        ...
        preds_standard = predictor.predict_first_section()
        ...
        print(f"     ✅ Время: {time_standard:.2f} сек, Predictions: {total_preds_standard}")
        
        # Метод 2: Через подсекции
        print("  ⏳ Метод 2: predict_first_section_via_subsections...")
        start_time = time.time()
        preds_subsections = predictor.predict_first_section_via_subsections()
        ...
        print(f"     ✅ Время: {time_subsections:.2f} сек, Predictions: {total_preds_subsections}")
...
print(f"Общее время обычного метода: {total_time_standard:.2f} сек")
print(f"Общее время через подсекции: {total_time_subsections:.2f} сек")

...
def render_dashboard():
    ...
    with st.sidebar:
        ...
        if not use_default_data and not use_experiment_data and not use_gdrive_data and not use_cloud_storage_data:
            ...
            # КРИТИЧНО: Проверяем, не загружены ли уже данные, чтобы избежать повторных rerun
            existing_predictions = safe_session_get("predictions_cloud", None)
            if existing_predictions and len(existing_predictions) > 0:
                st.info("📦 Данные уже загружены из облачного хранилища. Используйте их для анализа.")
            else:
